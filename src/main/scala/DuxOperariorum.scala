...
/** The worker controller class.
  ...
  */
class DuxOperariorum(gameCons: Game, selfCons: Player) extends BWAPIConnection{
  ...
  def getUnits(ownUnits: Buffer[ScUnit], neutralUnits: Buffer[ScUnit]) ={ //overrides abstract method
     for(i <- ownUnits){
  ...
  def getIdleWorkers(): ListBuffer[Operario] ={
    return idleWorkers
  }
